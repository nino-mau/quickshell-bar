...
Singleton {
    ...
    readonly property real cpuTempPerc: cpuTemp > 0 ? Math.max(0, Math.min(1, cpuTemp / 100)) : 0
    readonly property string displayCpuTemp: cpuTemp > 0 ? Math.round(cpuTemp) + "°" : "--"
    readonly property string displayCpuTempPerc: cpuTemp > 0 ? Math.round(cpuTempPerc * 100) + "%" : "--"
